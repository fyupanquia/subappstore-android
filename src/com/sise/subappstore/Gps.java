...
public class Gps {
	 ...
	 * 
	 * @param context
	 * @return retorna true si se logra registrar por lo menos un proveedor, de
	 *         lo contrario, falso
	 */
	public static boolean startListening(Context context) {
		...
		Log.v("GPS",lm.getAllProviders().toString());
		try {
		...
		if (gps_enabled) {
			lm.requestLocationUpdates(LocationManager.GPS_PROVIDER, updateTime,
					0, locationListener);
			Log.v("GPS","GPS Registrado");
			mensaje = "GPS Registrado";
		}
		if (network_enabled) {
			lm.requestLocationUpdates(LocationManager.NETWORK_PROVIDER,
					updateTime, 0, locationListener);
			Log.v("GPS","Network Registrado");
			mensaje = "Network Registrado";
		}
		return true;
	}

	/**
	 * @return retorna la mejor ubicacion obtenida dentro de los 2 minutos o la
	...
	static LocationListener locationListener = new LocationListener() {
		...
		public void onLocationChanged(Location location) {
			Log.v("GPS", "Location Recibido: " + location.getLatitude()
	...
	private static final int TWO_MINUTES = 15 * 1000;
